...
function logicChunks() {
	...
	if (v1 || v2) {
		...
		if (v1) {
			logServer("Chunks a ser adicionados: " + toFind.length + ". De _map" + _map.length + ", enviando map-orderChunks" + safeMapToSend.length + " " + getTime());
			...
			logServer("Apenas Ordem. De _map" + _map.length + ", ennviando map-orderChunks" + safeMapToSend.length + " " + getTime());
			setTimeout(() => { send({ type: "addMap-orderChunks", id: _id, pos: { x: _poss[0] / tamBlock, y: _poss[1] / tamBlock }, data: safeMapToSend }) }, 0);
		}else{
			erro("ERRO-001");
		}
...
function erro(code) {
	...
	document.getElementById("erro").textContent = code + ": "+erros.filter(e => e[0] == code)[0][1];
