...
function doMap() {
	...
	// Calcula viewport considerando o zoom
	// Com zoom, o viewport no mundo real fica menor/maior
...
function logicChunks() {
	...
	// 🛑 MUDANÇA AQUI: Criamos um array de adições
	let chunksToAdd = [];
	for (let i = 0; i < toFind.length; i++) {
	...
	// 🛑 APLICAÇÃO ASSÍNCRONA DOS CHUNKS 🛑
	if (chunksToAdd.length > 0) {
		...
		setTimeout(() => {
			...
			logServer("Chunks Adicionados: " + qtAdded + ". Enviando map-orderChunks" + _map.length + " " + getTime());
