...
async function processMessage(ws, data, client, fracId, sendGeral) {
	...
	switch (data.type) {
		...
		case "addInMap":
			...
			_map.map(c => {
				if (c.x == data.data.keyChunk.x && c.y == data.data.keyChunk.y) {
					...
					switch (data.data.localBlock) {
						case 0:
							c.chunk[data.data.localChunk.x][data.data.localChunk.y].unshift(data.data.what);
							break;
						default:
			...
			if (data.data.localChunk.x < 0 || data.data.localChunk.y < 0) {
				erro("Chunk com posição negativa não suportada --addInMap:" + JSON.stringify(data.data.localChunk));
			}
			let antes = chunkAtt.chunk[data.data.localChunk.x][data.data.localChunk.y];
			if (antes == null) {
			...
			chunkAtt.chunk[data.data.localChunk.x][data.data.localChunk.y] = novo;
...
function main() {
	if (!processingInMain["waterfall"]) {
		...
		setImmediate(async () => { // com bugs
			// try { console.log("in: " + JSON.stringify(_map.filter(c => c.x == 0 && c.y == 0)[0].chunk[21][6])); } catch (e) { }
			...
			if (ret.length > 0) {
				...
				let chunks = _map.filter(c => {
					for (let r of ret) {
						if (c.x == r.key.x && c.y == r.key.y) return true;
					}
					return false;
				});
...
async function waterfall() {
	...
	_map.map(chunk => {
		...
		for (var i = 0; i < chunk.chunk.length; i++) {
			...
			for (var j = 0; j < chunk.chunk[i].length; j++) {
				let blocks = chunk.chunk[i][j];

				for (var a = 0; a < blocks.length - 1; a++) { // -1 pra não ser o ultimo
					// if (chunkEqKey(chunk, "0,0") && i == 21 && j == 6) console.log("going-pre-inner(" + i + "," + j + "): " + JSON.stringify(chunk.chunk[i][j]));
					if (blocks[a].thing == "water") {
						...
						if (blocks[a + 1].depth >= p + v) {// cabe completamente
							...
							let holes = []
							for (let ad of adjs) {
								if (chunk.chunk[ad.i][ad.j].length > 0) {
									holes.push({ i: ad.i, j: ad.j });
								}
							}
							...
							for (let v = 1; v < ver.length; v++) {
								if (ver[v].block.length > 0) {
									let w = false;
									let ind = 0;
									for (let l = 0; l < ver[v].block.length; l++) {
										if (ver[v].block[l].thing == "water") {
											w = true;
											ind = l;
											break;
										}
									...
									if (w) {
										if (Math.round(Math.abs(ver[v].block[ind].depth - ver[0].block[0].depth) * variMin) / variMin > 1 / variMin) {
											quiet = false;
											// console.log(Math.round(Math.abs(ver[v].block[ind].depth - ver[0].block[0].depth)*variMin)/variMin);
										}
									}
								}
							}
							...
							let qt = holes.length;
							// level
							let levels = [{ h: 0 * variMin, d: chunk.chunk[i][j][1].depth * variMin, p: { i: i, j: j } }];
							for (let h of holes) {
								levels.push({ h: 0 * variMin, d: chunk.chunk[h.i][h.j][0].depth * variMin, p: { i: h.i, j: h.j } });
							}
							// ordena
							...
							for (let l of levels) {
								...
								if (base.height != 0) {
									if (l.p.i == i && l.p.j == j) {
										blocks[0] = base;
										chunk.chunk[i][j] = blocks;
										// console.log(i + " " + j + ":> " + JSON.stringify(chunk.chunk[21][6]));
									}
									else {
										chunk.chunk[l.p.i][l.p.j].unshift(base);
										// console.log(l.p.i + " " + l.p.j + ": " + JSON.stringify(chunk.chunk[l.p.i][l.p.j]));
									}
								}
							}
					...
					if (chunkEqKey(chunk, "0,0") && change && i == 21 && j == 6) {
						// console.log("going-pre: " + JSON.stringify(chunk.chunk[i][j]));
					}
				}
				chunk.chunk[i][j] = blocks;
			}
				...
				for (let blk of chunk.chunk[i]) {
					...
					for (let k = 0; k < blk.length; k++) {
						...
						if (b.thing == "water") {
							...
							vt += b.height;
							txt += t + ": " + JSON.stringify(b) + "\n";
						} else if (k > 0 && blk[k - 1].thing == "water") {
